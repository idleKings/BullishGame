...
function refreshData(){
    ...
    EGGS_TO_HATCH_1BULLS(function(eggs){
        eggstohatch1=eggs
    });
    getMyEggs(function(eggs){
        ...
        secondsuntilfull=eggstohatch1-eggs/lastNumBulls
        console.log('secondsuntilfull ',secondsuntilfull,eggstohatch1,eggs,lastNumBulls)
    ...
    getMyBulls(function(bulls){
        lastNumBulls=bulls
        var gfsdoc=document.getElementById('getfreebulls')
        if(bulls>0){
        ...
        var allnumbulls=document.getElementsByClassName('numbulls')
        for(var i=0;i<allnumbulls.length;i++){
            if(allnumbulls[i]){
                allnumbulls[i].textContent=translateQuantity(bulls,0)
            }
        }
        var productiondoc=document.getElementById('production')
        productiondoc.textContent=formatEggs(lastNumBulls*60*60)
    });
    updateBuyPrice()
    updateSellPrice()
	updateBullsmasterPrice()
	updateCurrentBullsmaster()
...
function updateEggNumber(eggs){
    var hatchbullsquantitydoc=document.getElementById('hatchbullsquantity')
    hatchbullsquantitydoc.textContent=translateQuantity(eggs,0)
...
function liveUpdateEggs(){
    if(lastSecondsUntilFull>1 && lastNumEggs>=0 && lastNumBulls>0 && eggstohatch1>0){
        currentTime=new Date().getTime()
        if(currentTime/1000-lastHatchTime>eggstohatch1){
            return;
        }
        difference=(currentTime-lastUpdate)/1000
        additionalEggs=Math.floor(difference*lastNumBulls)
...
function updateBullsmasterPrice(){
    var bullsmasterpricedoc=document.getElementById('bullsmasterprice')
	getBullsmasterReq(function(req) {
		bullsmasterpricedoc.textContent = translateQuantity(req, 0);
	});
}


function updateCurrentBullsmaster(){
    var currentbullsmasterdoc=document.getElementById('currentbullsmaster')
    //eggstobuydoc.textContent='?'
	currentbullsmaster.textContent=ceoAddress()
}

function getFreeBulls2(){
    var ethtospenddoc=0.001//document.getElementById('freebullsspend')
    weitospend=web3.toWei(ethtospenddoc,'ether')
    getFreeBulls(weitospend,function(){
...
function disableButtons(){
    var allnumbulls=document.getElementsByClassName('btn-lg')
    for(var i=0;i<allnumbulls.length;i++){
        if(allnumbulls[i]){
            allnumbulls[i].style.display="none"
        }
    }
}
function enableButtons(){
    ...
    for(var i=0;i<allnumbulls.length;i++){
        if(allnumbulls[i]){
            allnumbulls[i].style.display="inline-block"
        }
    }
}
